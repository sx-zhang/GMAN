...
class OfflineControllerWithSmallRotation(BaseController):
    ...
    def reset(self, scene_name=None):
        ...
        if scene_name != self.scene_name:
            ...
            if self.att_feature is not None:
                self.att_feature.close()
            self.att_feature = self.h5py.File(
                os.path.join(
                    self.offline_data_dir, self.scene_name, self.att_feature_file_name
                ),
                'r',
            )
    ...
    def get_att_feature(self):
        return self.att_feature[str(self.state)][()]
