...
def gman_val(
    ...
    while count < max_count:
        ...
        while True:
            ...
            if args.gradient_limit < 0 or episode_num < args.gradient_limit:
                ...
                num_gradients += 1
                if (args.model == 'EMLV3') or (args.model == 'EMLV3Base'):
                    # compute D loss
                    D_loss = compute_D_loss(args, player, gpu_id, model_options)
                    D_gradient = torch.autograd.grad(
                        D_loss["D_loss"],
                        [v for _, v in params_list[episode_num].items()],
                        allow_unused=True,
                    )
                    model_options.params = D_grad_step(model_options.params, D_gradient, args.D_lr)
                    params_list[episode_num] = D_grad_step(params_list[episode_num], D_gradient, args.D_lr)
        ...
        end_episode(
            ...
            EPA=len(player.episode.scene_states) / len(player.episode.environment.controller.grid),
            **reward_dict,
            **bucketed_spl,
        )
    ...
    # i = 0
    # print(model_options.params.keys())
    # for name, param in shared_model.named_parameters():
    #     room_meta_dict[name] = model_options.params[i]
    #     i += 1
    # eval_model = model_create_fn(args)
    # eval_model.load_state_dict(model_options.params)
    # state_dict =eval_model.state_dict()
    # torch.save(state_dict,'./{}.dat'.format(scene_type), _use_new_zipfile_serialization=False)
    ###
