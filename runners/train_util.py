...
def compute_D_loss(args, player, gpu_id, model_options):
    loss_hx = torch.cat((player.hidden[0], player.last_action_probs), dim=1)
    D_loss = {
        "D_loss": player.model.D_loss(
            loss_hx, player.learned_input, model_options.params
        )
    }
    return D_loss
...
def SGD_step(theta, grad, lr):
    ...
    for name, param in theta.items():
        if grad[j] is not None and "exclude" not in name and "ll" not in name and 'netD' not in name:
...
def D_grad_step(theta, grad, lr):
    theta_i = {}
    j = 0
    for name, param in theta.items():
        if grad[j] is not None and 'netD' in name:
            theta_i[name] = param - lr * grad[j]
        else:
            theta_i[name] = param
        j += 1
