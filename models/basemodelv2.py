...
class BaseModelv2(torch.nn.Module):
    def __init__(self, args):
        ...
        self.img_linear_0 = nn.Linear(512, 64)

        self.img_linear_1 = nn.Linear(2, 49)
        self.img_linear_2 = nn.Linear(49, 49)
        self.img_linear_3 = nn.Linear(49, 49)
        self.img_linear_4 = nn.Linear(49, 49)
        ...
        self.main_view40 = scio.loadmat('./data/main_view_GT/main_view40.mat')
        self.main_all_keys = self.main_view40.keys()
    ...
    def embedding(self, state, target, action_probs, params, att, target_feature):
        ...
        if params is None:
            if target_feature is None:
                ...
                self.fake_img = target_feature
            state_img = F.avg_pool2d(state, 7).view(1, 512)
            img_info = torch.cat((self.fake_img, state_img), dim=0)

            img_info = F.relu(self.img_linear_0(img_info))
            img_info = img_info.t()
            img_info = F.relu(self.img_linear_1(img_info))
            img_info = F.relu(self.img_linear_2(img_info))
            img_info = F.relu(self.img_linear_3(img_info))
            img_info = F.relu(self.img_linear_4(img_info))
            fake_reshaped = img_info.view(1, 64, 7, 7)
            ...
            x = torch.cat((x, glove_reshaped, action_reshaped, fake_reshaped), dim=1)
            ...
            img_info = F.relu(
                F.linear(
                    img_info,
                    weight=params["img_linear_0.weight"],
                    bias=params["img_linear_0.bias"],
                )
            )
            img_info = img_info.t()
            img_info = F.relu(
                F.linear(
                    img_info,
                    weight=params["img_linear_1.weight"],
                    bias=params["img_linear_1.bias"],
                )
            )
            img_info = F.relu(
                F.linear(
                    img_info,
                    weight=params["img_linear_2.weight"],
                    bias=params["img_linear_2.bias"],
                )
            )
            img_info = F.relu(
                F.linear(
                    img_info,
                    weight=params["img_linear_3.weight"],
                    bias=params["img_linear_3.bias"],
                )
            )
            img_info = F.relu(
                F.linear(
                    img_info,
                    weight=params["img_linear_4.weight"],
                    bias=params["img_linear_4.bias"],
                )
            )
    ...
    def forward(self, model_input, model_options):
        ...
        name = model_input.target_object+'_'+model_input.room_name.lower()
        ...
        if name in self.main_all_keys:
            main_view_feature = torch.from_numpy(self.main_view40[name].astype(np.float32)).to(
                torch.device(hx.device))
        else:
            main_view_feature =None

        target = model_input.target_class_embedding
        action_probs = model_input.action_probs
        params = model_options.params

        x, image_embedding = self.embedding(state, target, action_probs, params, att_episode, main_view_feature)
