...
from runners import nonadaptivea3c_val, gman_val
...
def main_eval(args, create_shared_model, init_agent):
    ...
    res_queue = mp.Queue()
    if args.model == "BaseModel" or args.model == "GCN" or args.model == "BaseModelv0" \
            or args.model == "BaseModelv1" or args.model == "BaseModelv2":
    ...
    try:
        ...
        for p in processes:
            time.sleep(0.1)
            p.join()
    model_dict = {"model": model_to_open}
    with open(args.results_json, "w") as fp:
        json.dump(tracked_means, fp, sort_keys=True, indent=4)
    with open(args.results_json.split('.')[0]+'_best_model.json', "w") as fp:
