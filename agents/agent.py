...
class ThorAgent:
    ...
    def action(self, model_options, training, demo=False):
        ...
        prev_feature = F.avg_pool2d(self.episode.prev_frame, 7).view(1, 512)
        current_feature = F.avg_pool2d(self.episode.current_frame, 7).view(1, 512)

        if self.learned_loss:
            if self.model_name == 'SAVN':
                res = torch.cat((self.hidden[0], self.last_action_probs), dim=1)  # savn
            elif self.model_name == 'EMLV1':
                res = torch.cat((self.last_action_probs, prev_feature, current_feature), dim=1)  # emlv1
            else:
                res = torch.cat((self.last_action_probs, prev_feature, out.fake_img), dim=1)  # emlv2
